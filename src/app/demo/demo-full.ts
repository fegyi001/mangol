...
export class DemoFullComponent implements OnInit {
  ...
  import { MangolConfig, MangolReady, MangolMapService } from 'mangol';

  declare var proj4: any;

  @Component({
    selector: 'mangol-demo-full',
    template: '<mangol [config]="config" (mapReady)="onMapReady($event)"></mangol>'
  })
  export class DemoFullComponent implements OnInit {
    config = {} as MangolConfig;
    ...
    onMapReady(evt: MangolReady) {
      this.mapService = evt.mapService;
